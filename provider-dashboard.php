...
// Get statistics (FIXED - earnings now counts completed bookings correctly)
$stats_sql = "SELECT 
    ...
    SUM(CASE WHEN b.booking_status = 'completed' THEN b.total_amount ELSE 0 END) as total_earnings,
    (SELECT COUNT(*) FROM provider_catalog WHERE provider_id = $provider_id AND is_active = 1) as total_services
    FROM bookings b WHERE b.provider_id = $provider_id";
$stats_result = $conn->query($stats_sql);
$stats = $stats_result->fetch_assoc();

// Get recent bookings
$bookings_sql = "SELECT b.*, u.full_name, u.phone, u.email,
                 CONCAT(b.house_number, ', ', b.street, ', ', b.area, ', Hyderabad - ', b.pincode) as address
                 FROM bookings b 
...
// If suspended, show prominent alert
if($provider_data['account_status'] == 'suspended') {
    // Format suspension date properly
    $suspension_date_text = '';
    if(!empty($provider_data['suspension_date']) && $provider_data['suspension_date'] != '0000-00-00 00:00:00') {
        $suspension_date_text = " on " . date('F d, Y', strtotime($provider_data['suspension_date']));
    }
    
    $suspension_reason = !empty($provider_data['suspension_reason']) ? htmlspecialchars($provider_data['suspension_reason']) : 'No reason provided';
    
    echo "
    <div style='background: linear-gradient(135deg, #fee2e2, #fecaca); border: 3px solid #ef4444; padding: 2rem; margin: 2rem; border-radius: 12px; text-align: center;'>
        <h2 style='color: #991b1b; margin-bottom: 1rem;'>
            <i class='fas fa-exclamation-triangle'></i> Account Suspended
        </h2>
        <p style='color: #7f1d1d; font-size: 1.1rem; margin-bottom: 1rem;'>
            Your provider account has been suspended{$suspension_date_text}
        </p>
        <p style='color: #7f1d1d; margin-bottom: 1.5rem;'>
            <strong>Reason:</strong> {$suspension_reason}
        </p>
        <a href='provider-notifications.php' style='background: #dc2626; color: white; padding: 1rem 2rem; border-radius: 8px; text-decoration: none; display: inline-block; font-weight: 600;'>
            <i class='fas fa-bell'></i> View Notifications & Next Steps
        </a>
    </div>
    ";
}
    ...
    <div class="dashboard-container">
        ...
        <aside class="sidebar">
            ...
            <ul class="sidebar-menu">
                ...
                <li><a href="provider-catalog.php"><i class="fas fa-list"></i> My Services</a></li>
            ...
            <div class="card">
                ...
                <?php if($bookings_result->num_rows > 0): ?>
                    <?php while($booking = $bookings_result->fetch_assoc()): 
                        ...
                        if(!empty($booking['start_time']) && !empty($booking['end_time'])) {
                            ...
                            $duration_text = $duration_hours . ' hour' . ($duration_hours != 1 ? 's' : '');
                    ...
                    <div class="booking-item">
                        ...
                        <div class="booking-details">
                            ...
                            <div>
                                <i class="fas fa-hourglass-half"></i> <?php echo $duration_text; ?>
                            </div>
                            <div>
                        ...
                        <div style="margin-top: 0.5rem;">
                            <strong>Special Requirements:</strong> <?php echo htmlspecialchars($booking['special_requests']); ?>
                        </div>
                        <?php endif; ?>
                ...
                <?php else: ?>
                    <div style="text-align: center; padding: 3rem; color: var(--text-light);">
                        ...
                        <p>No bookings yet. Add services to your catalog to start receiving bookings!</p>
                    </div>
                <?php endif; ?>
            </div>
        </main>
